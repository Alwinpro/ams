...
function updateUIForUser(user) {
    ...
    if (user) {
        ...
        if (authModal) authModal.style.display = 'none'; // Close modal on success
    } else {
        // Kiosk Mode
...
function setupNavigation() {
    ...
    const loginTrigger = document.getElementById('login-btn-nav');
    if (loginTrigger) {
    ...
    if (scanMode === 'face') {
        ...
        try {
            ...
            try {
                ...
                // If already playing, this might error or resolve, but we continue.
                // console.log("Play error/warning:", e);
            }
            const runFaceLoop = () => {
                const displaySize = { width: videoFeed.videoWidth, height: videoFeed.videoHeight };
                // Safety check for valid dimensions
                if (displaySize.width === 0 || displaySize.height === 0) {
                    setTimeout(runFaceLoop, 100); // Retry until ready
                    return;
                }
                try {
                    faceapi.matchDimensions(overlayCanvas, displaySize);
                } catch (e) { console.log("Dim Err", e); }
                const processFrame = async () => {
                        ...
                        if (faceMatcher) {
                            for (const detection of resizedDetections) {
                                ...
                                if (match.label !== 'unknown') {
                                    // Stop loop temporarily inside handleVerify? No, handleVerify handles logic.
                                    await handleVerify(match.label);
                                    // If handleVerify stopped scanning, we should break
                                    if (!isScanning) break;
                                }
                            }
                        }
                    ...
                    if (isScanning) setTimeout(processFrame, 100);
                };
                processFrame();
            };
            // Start logic
            if (videoFeed.readyState >= 1) {
                ...
                videoFeed.onloadedmetadata = () => runFaceLoop();
            }
        } catch (err) {
...
function playWarningBeep() {
    try {
        ...
        osc.frequency.exponentialRampToValueAtTime(330, ctx.currentTime + 0.2); // Descending tone
        gain.gain.setValueAtTime(0.1, ctx.currentTime);
...
// Lock to prevent race conditions from rapid video frames
const scanLocks = new Set();
async function handleVerify(scannedValue) {
    const now = Date.now();
    // 1. Instant Lock Check (Race Condition Preventer)
    if (scanLocks.has(scannedValue)) return;
    // 2. Cooldown Check (Logic Preventer)
    if (lastVerifyTime[scannedValue] && (now - lastVerifyTime[scannedValue] < COOLDOWN)) {
        // We still want to show the warning, but we need to fetch user details first to show it nicely.
        // However, if we let it pass here, we might spam fetches.
        // Optimization: If we HAVE details cached, show warning immediately?
        // For now, let's proceed but remember we are "locked" for this specific execution?
        // No, if we are in cooldown, we don't need to lock the async process because we might just return.
        // EXCEPT we want to show the warning popup, which is an async/UI process.
    }
    scanLocks.add(scannedValue);
    try {
        // ... (rest of logic) ...
        // Fetch User Details
        ...
        try {
            ...
            if (userDoc.exists) {
                ...
                if (lastVerifyTime[scannedValue] && (now - lastVerifyTime[scannedValue] < COOLDOWN)) {
                    const remaining = Math.ceil((COOLDOWN - (now - lastVerifyTime[scannedValue])) / 1000);
                    showScanResult(userPhoto, userName, empId, `Please wait ${remaining}s before rescanning.`, 'warning');
                    playWarningBeep();
                    // We must rely on showScanResult to clear the lock after the message is done.
                    return;
                }
            } else {
        ...
        try {
            ...
            // Query logs for this user (Client-side sort to avoid Missing Index error)
            const logsSnap = await db.collection('attendance_logs')
                .where('userId', '==', userId)
                .get();
            if (!logsSnap.empty) {
                // Manual Sort to find latest
                ...
                const lastTime = lastLog.timestamp ? lastLog.timestamp.toDate().getTime() : 0;
                if (lastLog.type === 'IN') {
                ...
                // DB has no records for this user -> First time IN
                // We TRUST the DB. If records were deleted, we start fresh with IN.
                type = 'IN';
                // Clear any local cache that might be stale and conflicting
                localStorage.removeItem('last_log_' + userId);
            }
        } catch (readErr) {
            console.warn("Could not read logs (Kiosk Mode?):", readErr);
            // Fallback to LocalStorage for Kiosk toggle
            // DISABLED: We want to rely ONLY on Firebase. If Firebase fails, we error out rather than using stale local data.
            // useLocalFallback = true; 
            console.error("Firebase Read Fail - Cannot verify previous state.");
            // Optional: Alert user connectivity issue?
            const lastLocal = null; // FORCE NULL. Do not check local storage.
        ...
        try {
            if (valid) {
                ...
                if (type === 'IN') playSuccessBeep();
                else playSuccessBeep(); // distinct sound for OUT? Standardizing for now.
                // Update Verify Timestamp for Cooldown
        ...
        scanLocks.clear(); // Ensure unlock on crash
    }
}
// --- Result Handling ---
function showScanResult(photo, name, empId, message, type) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            // Unlock scanning for everyone once UI clears
            scanLocks.clear();
    ...
    }, 4000); // 4 Seconds display time (Readability)
}
// --- Registration ---
function setupRegisterListeners() {
    ...
    // --- NEW: Close Camera & Remove Photo Logic ---
    ...
    if (signupForm) {
        signupForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (toggleAuthLink) toggleAuthLink.style.display = 'none';
            } catch (err) {
...
async function loadLogs() {
    ...
    // Auth Check
    if (!auth.currentUser) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; color: var(--text-muted);">Admin access required to view logs.</td></tr>';
        return;
    }
    try {
        ...
        snap.forEach(doc => {
            ...
            tr.id = `log-row-${doc.id}`;
            // 1. Employee ID
            const tdId = document.createElement('td');
            tdId.style.fontFamily = "'Courier New', monospace";
            tdId.textContent = d.empId || 'N/A';
            tr.appendChild(tdId);
            // 2. Name & Avatar
            const tdUser = document.createElement('td');
            const userDiv = document.createElement('div');
            userDiv.className = 'td-user';
            const avatarDiv = document.createElement('div');
            avatarDiv.style.cssText = "width: 32px; height: 32px; background: #4a90e2; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; color: white; font-size: 0.8rem;";
            avatarDiv.textContent = (d.name || '?').charAt(0).toUpperCase();
            ...
            badge.className = `status-badge ${d.type === 'IN' ? 'in' : 'out'}`;
            badge.textContent = d.type;
            tdType.appendChild(badge);
            tr.appendChild(tdType);
            // 4. Time
            const tdTime = document.createElement('td');
            tdTime.style.color = "#cbd5e1";
            tdTime.textContent = dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            tr.appendChild(tdTime);
            // 5. Date
            const tdDate = document.createElement('td');
            tdDate.style.color = "#94a3b8";
            tdDate.textContent = dateObj.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
            tr.appendChild(tdDate);
            // 6. Status
            const tdStatus = document.createElement('td');
            tdStatus.innerHTML = '<span style="color: #34d399; font-size: 0.85rem;"><i class="fas fa-check-circle"></i> Sync</span>'; // Icons are safe
            tr.appendChild(tdStatus);
            // 7. Actions (Delete)
            const tdAction = document.createElement('td');
            const btnDelete = document.createElement('button');
            btnDelete.style.cssText = "background: rgba(239, 68, 68, 0.1); border: none; color: #ef4444; width: 32px; height: 32px; border-radius: 8px; cursor: pointer; transition: 0.2s;";
            btnDelete.innerHTML = '<i class="fas fa-trash"></i>';
            btnDelete.addEventListener('click', () => deleteLog(doc.id));
...
async function loadUsers() {
    ...
    try {
        // Auth Check
        if (!auth.currentUser) {
            container.innerHTML = '<p style="color:var(--text-muted); text-align:center;">Admin access required to view users.</p>';
            return;
        }
        ...
        snap.forEach(doc => {
            ...
            div.className = 'user-card user-card-wrapper';
            div.setAttribute('data-name', data.name || '');
            div.setAttribute('data-empid', data.empId || ''); // Still attribute, but safe enough if not rendered as HTML
            // Image
            const img = document.createElement('img');
            img.src = photoUrl;
            img.alt = "User Photo"; // Generic alt to avoid XSS in alt text if rendered poorly (though alt is usually safe)
            // Info Container
            const infoDiv = document.createElement('div');
            infoDiv.className = 'u-info';
            const h4 = document.createElement('h4');
            h4.textContent = data.name || 'Unknown'; // Safe Text
            const p = document.createElement('p');
            p.textContent = `ID: ${data.empId || 'N/A'}`; // Safe Text
            ...
            btnDelete.innerHTML = '<i class="fas fa-trash"></i>'; // Icon is safe
            btnDelete.addEventListener('click', () => deleteUser(doc.id, data.name)); // Safe closure
            // Assemble
            div.appendChild(img);
